...
rv_pred = np.load('./pred_with_busmeta.npy')
cnn_pred  = np.load('./cnnprediction.npy')
#cnn_pred_score = np.load('./cnn_pred_score.npy')
...
print "rv_pred.shape",rv_pred.shape
print "cnn_pred.shape",cnn_pred.shape
...
for i in range(len(cnn_pred)):
    if cnn_pred[i][0] in x_test_id:
        cnn_pred_dict[cnn_pred[i][0]] = cnn_pred[i][1]
print "size of cnn_pred_dict on test set",len(cnn_pred_dict)

rv_pred_dict = dict()
for i in range(len(rv_pred)):
    if rv_pred[i][0] in x_test_id:
        rv_pred_dict[rv_pred[i][0]] = rv_pred[i][1]
...
only_rv_csv = csv.writer(open("only_rv.csv",'w'),delimiter=',')
only_cnn_csv = csv.writer(open("only_cnn.csv",'w'),delimiter=',')

for key in rv_pred_dict:
    rv_pred_score = int(rv_pred_dict[key])
    cnn_pred_score = int(float(cnn_pred_dict[key]))
    final_pred = combinePred(rv_weight,cnn_weight,rv_pred_score,cnn_pred_score,cutoff)
    ...
    if final_pred == labels[key]:
        ...
        if final_pred==1:
            ...
            FN+=1
    if rv_pred_score != cnn_pred_score:
        if(labels[key] == rv_pred_score):
            only_rv_correct[rv_pred_score] +=1
            only_rv_id.append(key)
            only_rv_csv.writerow([key,rv_pred_score])
        if(labels[key]== cnn_pred_score):
            only_cnn_correct[cnn_pred_score] += 1
            only_cnn_id.append(key)
            only_cnn_csv.writerow([key,cnn_pred_score])
...
print ("only rv",only_rv_id[0])
print ("only cnn",only_cnn_id[0])
